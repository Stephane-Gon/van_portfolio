...
const RouteLink = ({ link, activeLink, isMobile = false }: RouteLinkProps) => {
  ...
  return (
    <Link href={link.href} key={`sidebar-main-link-id-${link.id}`} className='pr-2 flex items-center justify-start gap-2 group'>
      <span className={`bg-text/50 p-1.5 ${activeLink === link.id ? 'rounded-md' : 'rounded-full'} group-hover:rounded-md transition-all`}>
        {
          link.styles.fill ?
      ...
      <p className={`${ isMobile ? 'inline' : 'hidden @3xs/sidebar:inline'} text-xl transition-all leading-none ${activeLink === link.id ? 'text-tertiary' : 'text-primary'}`}>{link.label}</p>
    </Link>
  )
}
